...
def stageA(clock, arrival_list_precheck, stageA_precheck, stageA_precheck_officer, stageB_precheck, mean_A, time_limit):
    ...
    test_cnt = 0
    outside_queue = init_queue_list(len(arrival_list_precheck))
    ...
    for i in list(range(0, len(stageA_precheck), 1)):
        stageA_precheck[i] = jqueue(stageA_precheck[i], clock, time_limit, outside_queue[i])
...
def stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray):
    ...
    for i in list(range(0, len(stageB_precheck), 1)):
        if len(stageB_precheck[i]) > 0 and stageB_precheck_pane[i] is None:
            ...
            mu_B = reject_zero(round(max(mu_mili / cnt_milimeter, (stageB_precheck_pane[i].num_boxes * mu_x_ray) / cnt_x_ray)))
            # print(mu_B)
...
def airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger, cnt_officer_precheck, cnt_officer_regular, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray):
    ...
    while True:
        ...
        if cnt_finished_passenger == total_passenger or clock == total_time_scale:
            break;
        test_cnt_precheck = stageA(clock, arrival_list_precheck, stageA_precheck, stageA_precheck_officer, stageB_precheck, mean_A, time_limit)
        test_cnt_regular = stageA(clock, arrival_list_regular, stageA_regular, stageA_regular_officer, stageB_regular, mean_A, time_limit)
        finish_list_precheck = stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray)
        finish_list_regular = stageB(clock, stageB_regular, stageB_regular_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray)
        clock += 1
...
def overall_mean_variance(iteration, TSA_precheck_arrival_time, Regular_arrival_time, num_flight, flight_gap_time, total_passenger, p_precheck_rate, k_average_box_precheck, k_average_box_regular, cnt_officer_precheck, cnt_officer_regular, num_hour, time_limit, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray):
    mean_list = []
    variance_list = []
    ttt_list = []
    for i in range(iteration):
        ...
        result_list, ttt = airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger,
                              ...
                              Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray)
