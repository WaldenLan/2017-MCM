...
def serve_MMS(officer, queue, clock, mean_A, stage_B=-1, cnt_milimeter=-1, cnt_x_ray=-1, cnt_x_ray_backup=-1, boxnum_limit=-1):
    ...
    while True:
        ...
        if state is True:
            ...
            if stage_B == -1:
                ...
                ratio_x_ray = 0
                if queue[0].num_boxes >= boxnum_limit:
                    ratio_x_ray = cnt_x_ray_backup
                else:
                    ratio_x_ray = cnt_x_ray

                mu_B = reject_zero(round(max(mu_mili / cnt_milimeter, (queue[0].num_boxes * mu_x_ray) / (ratio_x_ray * cnt_milimeter))))
                # print(mu_B)
...
def stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit):
    ...
    serve_MMS(stageB_precheck_pane, stageB_precheck, clock, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit)
...
def airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger, cnt_officer_precheck, cnt_officer_regular, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, threshold, cnt_extra_officer_precheck, cnt_extra_officer_regular, cnt_extra_pane_precheck, cnt_x_ray_backup, boxnum_limit):
    ...
    while True:
        ...
        if cnt_finished_passenger == total_passenger or clock == total_time_scale:
            break;
        # Simulate the Peak Case
        if threshold != -1 and clock == threshold:
            for i in range(cnt_extra_officer_precheck):
                stageA_precheck_officer.append(None)
            for i in range(cnt_extra_officer_regular):
                stageA_regular_officer.append(None)
            for i in range(cnt_extra_pane_precheck):
                stageB_precheck_pane.append(None)
            for i in range(cnt_extra_pane_precheck * ratio_regular_over_precheck):
                stageB_regular_pane.append(None)
        ...
        finish_list_precheck = stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time/cnt_pane_precheck, X_ray_scan_time/cnt_pane_precheck, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit)
        finish_list_regular = stageB(clock, stageB_regular, stageB_regular_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_regular, Milimeter_scan_time/(cnt_pane_precheck*ratio_regular_over_precheck), X_ray_scan_time/(cnt_pane_precheck*ratio_regular_over_precheck), cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit)
        clock += 1
...
# Arrival rate based on Normal Distribution
def get_arrival_list_2(norm_mean, norm_std_dev, mean_arrival_time, num_flight, flight_gap_time, total_passenger, mean_num_boxes, cnt_officer, num_hour):
    # l = init_queue_list(cnt_officer)
    l = []
    arrival_time = 0
    total_time_scale = num_hour * 3600
    for i in range(round(total_passenger)):
        flight_id   = 0
        while True:
            flight_id = round(np.random.normal(norm_mean, norm_std_dev))
            if flight_id == 0:
                flight_id = 1
            elif flight_id == num_flight:
                flight_id = num_flight - 1
            elif flight_id < 0 or flight_id > num_flight:
                continue
            else:
                break
...
def overall_mean_variance(iteration, TSA_precheck_arrival_time, Regular_arrival_time, num_flight, flight_gap_time, total_passenger, p_precheck_rate, k_average_box_precheck, k_average_box_regular, cnt_officer_precheck, cnt_officer_regular, num_hour, time_limit, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, norm_mean, norm_std_dev, cnt_extra_officer_precheck, cnt_extra_officer_regular, cnt_extra_pane_precheck, cnt_x_ray_backup, boxnum_limit):
    mean_list = []
    variance_list = []
    ttt_list = []
    ...
    threshold = -1
    if norm_mean != -1:
        threshold = (norm_mean - norm_std_dev) * (round(flight_gap_time * 3600))

    for i in range(iteration):
        print("Current-iteration #: " + str(i))
        arrival_list_precheck   = []
        arrival_list_regular    = []
        if norm_mean == -1:
            ...
            arrival_list_precheck = get_arrival_list_2(norm_mean, norm_std_dev, TSA_precheck_arrival_time, num_flight, flight_gap_time,
                                                     total_passenger * p_precheck_rate, k_average_box_precheck,
                                                     cnt_officer_precheck, num_hour)
            arrival_list_regular = get_arrival_list_2(norm_mean, norm_std_dev, Regular_arrival_time, num_flight, flight_gap_time,
                                                total_passenger * (1 - p_precheck_rate), k_average_box_regular,
                                                cnt_officer_regular, num_hour)
        ...
        result_list, ttt = airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger,
                              ...
                              Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, threshold, cnt_extra_officer_precheck,
                              cnt_extra_officer_regular, cnt_extra_pane_precheck, cnt_x_ray_backup, boxnum_limit)
    ...
    print("Final Mean: " + str(mean[0]) + " +/- " + str(math.sqrt(mean[1])))
    print("Final Variance: " + str(variance[0]) + " +/- " + str(math.sqrt(variance[1])))
    print("Final PASS Rate: " + str(ttt[0] * 100) + " +/- " + str(math.sqrt(ttt[1] * 100)) + "%")
...
# norm_mean, norm_std_dev = -1, -1
norm_mean, norm_std_dev = 11, 4


cnt_extra_officer_precheck = 0
cnt_extra_officer_regular = 0
cnt_extra_pane_precheck = 0
...
# New Feature:
boxnum_limit                = 5
# Ratio milimeter/x-ray = 1/(cnt_x_ray_backup or cnt_x_ray)
cnt_x_ray_backup            = 2
