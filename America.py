...
def serve_MMS(officer, queue, clock, mean_A, stage_B=-1, cnt_milimeter=-1, cnt_x_ray=-1):
    ...
    while True:
        ...
        if state is True:
            ...
            if stage_B == -1:
                ...
                mu_mili = data_generator(mean_A, 1, single=1) + data_generator(2, 1, single=1)
                mu_x_ray = data_generator(stage_B, 1, single=1) + data_generator(1.5, 1, single=1)
                mu_B = reject_zero(
                    round(max(mu_mili/cnt_milimeter, (queue[0].num_boxes * mu_x_ray) / cnt_x_ray)))
                # print(mu_B)
...
def stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray):
    ...
    serve_MMS(stageB_precheck_pane, stageB_precheck, clock, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray)
...
def airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger, cnt_officer_precheck, cnt_officer_regular, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, threshold, cnt_extra_officer_precheck, cnt_extra_officer_regular, cnt_extra_pane_precheck):
    ...
    while True:
        ...
        finish_list_precheck = stageB(clock, stageB_precheck, stageB_precheck_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_precheck, Milimeter_scan_time/cnt_pane_precheck, X_ray_scan_time/cnt_pane_precheck, cnt_milimeter, cnt_x_ray)
        finish_list_regular = stageB(clock, stageB_regular, stageB_regular_pane, mean_C, mean_D, total_time_scale, p_unsafe_rate_regular, Milimeter_scan_time/(cnt_pane_precheck*ratio_regular_over_precheck), X_ray_scan_time/(cnt_pane_precheck*ratio_regular_over_precheck), cnt_milimeter, cnt_x_ray)
        clock += 1
...
def overall_mean_variance(iteration, TSA_precheck_arrival_time, Regular_arrival_time, num_flight, flight_gap_time, total_passenger, p_precheck_rate, k_average_box_precheck, k_average_box_regular, cnt_officer_precheck, cnt_officer_regular, num_hour, time_limit, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, norm_mean, norm_std_dev, cnt_extra_officer_precheck, cnt_extra_officer_regular, cnt_extra_pane_precheck):
    mean_list = []
    variance_list = []
    ttt_list = []
    ...
    for i in range(iteration):
        ...
        result_list, ttt = airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger,
                              ...
                              cnt_extra_officer_regular, cnt_extra_pane_precheck)
