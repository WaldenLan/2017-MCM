    ...
    while True:
        ...
        if state is True:
            ...
            if stage_B == -1:
                ...
                mu_B = reject_zero(
                    round(max(mu_mili / cnt_milimeter, (queue[0].num_boxes * mu_x_ray) / (ratio_x_ray * cnt_milimeter))))
                # print(mu_B)
...
def airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger, cnt_officer_precheck, cnt_officer_regular, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit):
...
def overall_mean_variance(iteration, TSA_precheck_arrival_time, Regular_arrival_time, num_flight, flight_gap_time, total_passenger, p_precheck_rate, k_average_box_precheck, k_average_box_regular, cnt_officer_precheck, cnt_officer_regular, num_hour, time_limit, cnt_pane_precheck, ratio_regular_over_precheck, mean_A, mean_C, mean_D, p_unsafe_rate_precheck, p_unsafe_rate_regular, Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit):
    mean_list = []
    variance_list = []
    ttt_list = []
    for i in range(iteration):
        ...
        result_list, ttt = airport(arrival_list_precheck, arrival_list_regular, num_hour, time_limit, total_passenger,
                              ...
                              Milimeter_scan_time, X_ray_scan_time, cnt_milimeter, cnt_x_ray, cnt_x_ray_backup, boxnum_limit)
